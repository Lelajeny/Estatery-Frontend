...
// List of all settings tabs – each has an id (used in URL) and a label for the sidebar
const SETTINGS_NAV = [
...
// Creates a shallow copy of the profile so we can edit it without mutating the original
...
// Only these section IDs are allowed – used to validate the ?section= URL param
...
export default function Settings() {
  // Read ?section= from URL so we can deep-link to a specific tab
  ...
  // Draft holds the user's edits before they hit Save – we don't commit until they confirm
  ...
  // When user switches to My Account or Password, reset the draft so we show fresh data
  useEffect(() => {
  ...
  // Updates one or more fields in the My Account draft – used by form inputs
  const handleUpdateDraft = (partial: Partial<UserProfile>) => {
    setDraft((prev) => ({ ...prev, ...partial }));
  };

  // Same idea as handleUpdateDraft but for the password fields
  ...
  // Returns a user-friendly message for the password section – either an error or "Your changes will be saved"
  ...
  // Called when user clicks Confirm in the save modal – runs the right save handler for the section, then closes modal
  ...
  // Only enable Save when we're not on password section, OR when password fields are valid
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {/* Main content: sidebar nav on left, content area on right */}
      ...
      {/* Save confirmation modal – rendered via portal so it appears above everything and doesn't mess with layout */}
      {saveConfirmOpen &&
