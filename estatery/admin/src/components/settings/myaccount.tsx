...
// User type options for the dropdown – customer, owner, admin
...
export function MyAccount({ draft: draftProp, onUpdateDraft }: MyAccountProps) {
  ...
  // In Settings we use draft + onUpdateDraft; standalone we use profile + updateProfile
  const draft = draftProp ?? profile;
  const update = onUpdateDraft ?? updateProfile;

  // When user picks an image file, read it as base64 and pass to update
