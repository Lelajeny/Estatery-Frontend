...
/** Parse date string to timestamp for sorting. Returns 0 if invalid. */
...
export function PropertyListingTable({ properties }: PropertyListingTableProps) {
  ...
  const [filterListingType, setFilterListingType] = React.useState<"all" | ListingTypeApi>("all");
  const [filterStatus, setFilterStatus] = React.useState<"all" | PropertyStatusApi>("all");

  /** Filter by search (title, location, id), property_type, listing_type, status */
  const filtered = React.useMemo(() => {
    ...
    if (filterListingType !== "all") {
      result = result.filter((p) => (p.listing_type ?? "rent") === filterListingType);
    }
    if (filterStatus !== "all") {
      result = result.filter((p) => p.status === filterStatus);
    }
    return result;
  }, [properties, search, filterType, filterListingType, filterStatus]);

  /** Sort filtered list by Last Updated or Price, asc/desc */
  ...
  React.useEffect(() => setPage(1), [search, filterType, filterListingType, filterStatus]);
          ...
          <div className="relative">
            <Button
              ...
              className={cn(
                ...
                (filterType !== "all" || filterListingType !== "all" || filterStatus !== "all") && "border-[var(--logo)] bg-[var(--logo-muted)]"
              )}
            >
              <Filter className="mr-1.5 size-4" />
              Filter
              {(filterType !== "all" || filterListingType !== "all" || filterStatus !== "all") && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(["all", "rent", "sale"] as const).map((opt) => (
                      <button
                        key={opt}
                        type="button"
                        onClick={() => setFilterListingType(opt)}
                        className={cn(
                          "rounded-lg px-3 py-1.5 text-sm",
                          filterListingType === opt
                            ? "bg-[var(--logo)] text-white"
                            : "bg-[#f1f5f9] text-[#475569] hover:bg-[#e2e8f0]"
                        )}
                      >
                        {opt === "all" ? "All" : opt === "rent" ? "For Rent" : "For Sale"}
                      </button>
                    ))}
                  </div>
                  <p className="mb-3 mt-4 text-xs font-semibold uppercase tracking-wide text-[#64748b]">
        ...
        <table className="w-full min-w-[800px] text-left text-sm">
          <thead>
            <tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
              ...
              <th className="px-3 py-2 font-medium text-[#64748b] sm:px-4">Rent / Sale</th>
          ...
          <tbody>
            {pageProps.map((prop) => (
              ...
              >
                ...
                <td className="px-3 py-2 sm:px-4">
                  <span className="inline-flex rounded bg-[#e0f2fe] px-1.5 py-0.5 text-[10px] font-medium text-[#0369a1]">
                    {getListingTypeDisplay(prop.listing_type)}
                  </span>
                </td>
